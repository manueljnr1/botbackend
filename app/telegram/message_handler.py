# app/telegram/message_handler.py
"""
Telegram Message Handler
Processes incoming Telegram messages and generates responses
"""
...
class TelegramMessageHandler:
    """
    Handles incoming Telegram messages and generates appropriate responses
    """
    
    def __init__(self, db: Session):
        self.db = db
        self.chatbot_engine = ChatbotEngine(db)
    ...
    async def _handle_message(self, message: Dict[str, Any], 
        ...
        try:
            ...
            response_data = await self._process_with_chatbot(text, chat_record, integration)
    ...
    async def _process_with_chatbot(self, text: str, chat_record: TelegramChat, 
        ...
        try:
            ...
            # Process with chatbot engine using simple memory
            result = self.chatbot_engine.process_message_simple_memory(
            ...
            logger.error(f"Error processing with chatbot: {e}")
    ...
    def _create_response_keyboard(self, response_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Create inline keyboard based on response"""
        # Check if response suggests follow-up actions
        response_text = response_data.get("response", "").lower()
        
        # Add helpful quick action buttons for certain responses
        if any(word in response_text for word in ["setup", "configure", "install", "guide"]):
    ...
    async def _process_unknown_command(self, command: str, chat_id: str, chat_record: TelegramChat,
                                      integration: TelegramIntegration, 
                                      telegram_service: TelegramService) -> bool:
        """Process unknown commands as regular messages"""
        # Remove the / and treat as regular message
        text = command[1:] if command.startswith("/") else command
        return await self._process_with_chatbot(text, chat_record, integration)
...
I'm your AI assistant, here to help you with any questions you might have.
...
*Need more help?*
I'm an AI assistant for {company_name}. For complex issues, I can connect you with human support.
