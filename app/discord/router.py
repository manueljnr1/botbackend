...
async def discord_oauth_authorize(
    api_key: str = Header(..., alias="X-API-Key"),
    db: Session = Depends(get_db)
):
    tenant = get_tenant_from_api_key(api_key, db)
...
async def discord_oauth_callback(
    ...
    async with httpx.AsyncClient() as client:
        ...
        was_enabled = getattr(tenant, 'discord_enabled', False)
        tenant.discord_bot_token = tokens['access_token']
        tenant.discord_application_id = bot_data['application']['id']
        tenant.discord_enabled = True
        tenant.discord_refresh_token = tokens.get('refresh_token')
        db.commit()
        ...
        if background_tasks:
            background_tasks.add_task(restart_bot_for_tenant, tenant_id)
    
    return RedirectResponse(url=f"/settings?discord=success")
