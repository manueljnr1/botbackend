...
class SupabaseStorageService:
    ...
    def __init__(self):
        # Check if Supabase is configured (including dummy values)
        if (not settings.SUPABASE_URL or 
            not settings.SUPABASE_SERVICE_KEY or
            settings.SUPABASE_URL == "https://dummy.supabase.co" or
            settings.SUPABASE_SERVICE_KEY == "dummy_key_for_development"):
            
            logger.warning("⚠️ Supabase not configured or using dummy values, storage features disabled")
            self.client = None
            self.knowledge_base_bucket = "knowledge-base-files"
            self.vector_store_bucket = "vector-stores"
            return
    ...
    def upload_file(self, bucket: str, path: str, file_content: bytes) -> bool:
        """Upload file to Supabase Storage"""
        if not self.client:
            logger.warning("Storage not available, cannot upload file")
            return False
    ...
    def download_file(self, bucket: str, path: str) -> bytes:
        """Download file from Supabase Storage"""
        if not self.client:
            raise Exception("Storage not available, cannot download file")
    ...
    def delete_file(self, bucket: str, path: str) -> bool:
        """Delete file from Supabase Storage"""
        if not self.client:
            logger.warning("Storage not available, cannot delete file")
            return False
    ...
    def delete_folder(self, bucket: str, folder_path: str) -> bool:
        """Delete all files in a folder"""
        if not self.client:
            logger.warning("Storage not available, cannot delete folder")
            return False
    ...
    def download_to_temp(self, bucket: str, path: str) -> str:
        """Download file to temporary location and return path"""
        if not self.client:
            raise Exception("Storage not available, cannot download to temp")
    ...
    def upload_knowledge_base_file(self, tenant_id: int, filename: str, content: bytes) -> str:
        """Upload knowledge base file and return cloud path"""
        if not self.client:
            raise Exception("Storage not available, cannot upload knowledge base file")
    ...
    def upload_vector_store_files(self, tenant_id: int, vector_store_id: str, local_dir: str):
        """Upload all vector store files from local directory"""
        if not self.client:
            raise Exception("Storage not available, cannot upload vector store files")
    ...
    def download_vector_store_files(self, tenant_id: int, vector_store_id: str) -> str:
        """Download vector store files to temp directory and return path"""
        if not self.client:
            raise Exception("Storage not available, cannot download vector store files")
    ...
    def delete_vector_store(self, tenant_id: int, vector_store_id: str) -> bool:
        """Delete all vector store files"""
        if not self.client:
            logger.warning("Storage not available, cannot delete vector store")
            return False
...
class LogoUploadService:
    def __init__(self):
        ...
        if (not settings.SUPABASE_URL or 
            ...
            logger.warning("⚠️ Supabase not configured, logo upload service disabled")
            self.supabase = None
            self.bucket_name = settings.SUPABASE_STORAGE_BUCKET or "tenant-logos"
            self.max_size = settings.MAX_LOGO_SIZE or 2 * 1024 * 1024  # 2MB
            self.allowed_types = [
        ...
        try:
            ...
            logger.info("✅ Logo upload service initialized")
            
        except Exception as e:
            logger.error(f"❌ Failed to initialize logo upload service: {e}")
            self.supabase = None
    ...
    async def upload_logo(self, tenant_id: int, file: UploadFile) -> Tuple[bool, str, Optional[str]]:
        ...
        if not self.supabase:
            return False, "Logo upload service not available (Supabase not configured)", None
    ...
    async def delete_logo(self, logo_url: str) -> bool:
        """Delete logo from Supabase Storage"""
        if not self.supabase:
            logger.warning("Logo upload service not available, cannot delete logo")
            return False
...
try:
    ...
    if storage_service.client is None:
        logger.info("📦 Storage service running in dummy mode (Supabase not configured)")
    else:
        logger.info("✅ Storage service initialized with Supabase")
except Exception as e:
    logger.error(f"❌ Storage service initialization failed: {e}")
    
    # Create a dummy storage service that gracefully handles errors
